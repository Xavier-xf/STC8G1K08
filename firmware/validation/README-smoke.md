...
`RVMDK/STC8G1K08-RESET.uvproj` 是外部复位阶段的独立工程，保持 SMOKE 和 MONITOR 工程不变。

- 输出文件：`RVMDK/list/stc8g1k08_reset.hex`
- P5.4/INT2：继续检测 CPU_CHECK 下降沿
- P5.5：正常状态高阻，超时后按 `AP_RESET_ASSERT_LEVEL` 驱动 200 ms
- RESET 启动宽限：30 s，避免 V853 正常启动期间误复位
- 内部 WDT：24 MHz、128 分频，约 2.1 s；主循环负责清 WDT

烧录后先确认正常心跳输出 `state=monitoring output=high-z`，再执行 `kill -STOP <实际PID>` 验证一次性复位脉冲。当前跳过了 P5.5/AP-RESET 示波器测量，`AP_RESET_ASSERT_LEVEL=0` 是待现场确认的低有效假设。
